...
class VPAutoComplete: UIView {
    ...
    public var dataSource : [CountryModel] = [CountryModel]()
    ...
    var filterDataSource : [CountryModel] = [CountryModel]()
    ...
    // It will add extra width to auto - complete field (Right). you can adjust from here.
    var extraWidth: CGFloat = 10
    ...
    public convenience init(onTextField: UITextField , dataSource : [CountryModel], onView: UIView, completionHandler: @escaping CompletionHandler){
        self.init()
    ...
    public func show(completionHandler: @escaping CompletionHandler){
        ...
        if (textFieldView != nil) {
            self.tableView?.frame = CGRect(x: 0, y: 0, width: textFieldView.frame.width + extraWidth, height: 150)
            self.frame = CGRect(x: onTextField.frame.minX, y: textFieldView.frame.maxY, width: textFieldView.frame.width + extraWidth, height: 150)
        }else{
            self.tableView?.frame = CGRect(x: 0, y: 0, width: onTextField.frame.width + extraWidth, height: 150)
            self.frame = CGRect(x: onTextField.frame.minX, y: onTextField.frame.maxY, width: onTextField.frame.width + extraWidth, height: 150)
        }
    ...
    @objc func didChangeText(textField: UITextField) {
        if let textInField = onTextField.text {
            if textInField.count == 0{
                ...
                if isFilterByName{
                    is_filter = true
                    let tempData = dataSource.filter { ($0.countryName?.localizedCaseInsensitiveContains(textInField))! }
                    filterDataSource = tempData
                    self.isShowView(is_show: true)
                    self.tableView?.reloadData()
                }else{
                    is_filter = true
                    let tempData = dataSource.filter { ($0.countryCode?.localizedCaseInsensitiveContains(textInField))! }
                    filterDataSource = tempData
                    self.isShowView(is_show: true)
                    self.tableView?.reloadData()
                }
...
extension VPAutoComplete : UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if is_filter{
            ...
            let selectedIndex = dataSource.index(of: selectedCountry)
            self.isShowView(is_show: false)
            self.completionHandler!(selectedCountry.countryCode!, selectedIndex!)
            return
        }
        let selectedCountry = dataSource[indexPath.row]
...
extension VPAutoComplete : UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if is_filter {
            countryModel = filterDataSource[indexPath.row]
        }else{
            countryModel = dataSource[indexPath.row]
        }
        cell.lblTitle.text = countryModel?.countryCode
        cell.iconView.image = UIImage(named: (countryModel?.flagIcon)!)
        cell.lblSubTitle.text = countryModel?.countryName
        if isSearchBig {
    ...
    @objc func keyboarDidShown(notification: NSNotification){
        ...
        if (self.onTextField) != nil {
            ...
            if aRect.height < (self.frame.origin.y + self.frame .height) {
                if !isOnTop {
                    if (textFieldView != nil) {
                        self.frame = CGRect(x: textFieldView.frame.minX, y: self.frame.origin.y - self.frame.height - self.textFieldView.frame.height, width: textFieldView.frame.width + extraWidth, height: self.frame.height)
                    }else{
                        self.frame = CGRect(x: onTextField.frame.minX, y: self.frame.origin.y - self.frame.height - self.onTextField.frame.height, width: onTextField.frame.width + extraWidth, height: self.frame.height)
                    }
